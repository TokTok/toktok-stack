...
def _python_config(repository_ctx, versions):
    for version in versions:
        ...
        if result.return_code == 0:
            ...
            def execute_python_config(*args):
                return repository_ctx.execute(python_config + args)
            return execute_python_config, version
...
def _impl(repository_ctx):
    ...
    includes_result = python_config("--includes")
    ...
    if repository_ctx.path("/usr/include/x86_64-linux-gnu").exists:
        abiflags_result = python_config("--abiflags")
