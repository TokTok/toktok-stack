...
def java_junit5_test(name, srcs, test_package, deps = [], runtime_deps = [], **kwargs):
    """Build a java_test for JUnit 5 with Jupiter.

    Args:
      name: Rule name.
      srcs: Java sources.
      test_package: The Java package all the test classes are contained in.
      deps: Java dependencies.
      runtime_deps: Dependencies that need to be present at runtime.
      **kwargs: Other parameters passed to java_test.
    """
    FILTER_KWARGS = [
        "main_class",
        "use_testrunner",
