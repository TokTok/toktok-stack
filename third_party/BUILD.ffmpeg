...
asm_library(
    ...
    hdrs = [
        ...
        "@toktok//third_party/ffmpeg:config_asm",
    ],
    archive = False,
    asmopts = [
        "-Iexternal/ffmpeg",
        ...
        "@toktok//tools/config:freebsd": [],
        "@toktok//tools/config:linux": [],
        "@toktok//tools/config:osx": ["-DPREFIX"],
        "@toktok//tools/config:windows": [],
    }),
)
...
genrule(
    ...
    cmd = """
        ...
        ./configure --cc="$(CC)" || (cat ffbuild/config.log && false)
        DIR=`pwd`
        cd -
        for i in $(OUTS); do
...
alias(
    name = "ffmpeg_sysdep",
    actual = select({
        "@toktok//tools/config:freebsd": ":ffmpeg_freebsd",
        "@toktok//tools/config:linux": ":ffmpeg_linux",
        "@toktok//tools/config:osx": ":ffmpeg_osx",
        "@toktok//tools/config:windows": ":ffmpeg_windows",
    }),
)
...
cc_library(
    ...
    srcs = [
        ...
        "@toktok//tools/config:freebsd": [],
        "@toktok//tools/config:linux": [],
        "@toktok//tools/config:osx": ["fftools/ffmpeg_videotoolbox.c"],
        "@toktok//tools/config:windows": [],
    }),
    copts = COPTS,
    deps = [
